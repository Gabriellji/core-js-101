...
function removeFirstOccurrences(str, value) {
  ...
  const c = str.slice(0, a);
  const d = str.slice(b, str.length);
  return c + d;
}
...
function getRectangleString(width, height) {
  ...
  for (let l = 0; l < height; l += 1) {
    ...
    for (let h = 0; h < width; h += 1) {
      ...
      if (l === height - 1 && h === width - 1) {
        line += '┘';
      }


      if ((l === 0 || l === height - 1) && h > 0 && h < width - 1) {
        line += '─';
      }
      ...
      if ((h === 0 || h === width - 1) && l > 0 && l < height - 1) {
        line += '│';
      }
    }
...
function encodeToRot13(str) {
  ...
  return arr.map((el) => {
    ...
    letter += 13;
    letter = letter > 90 ? letter - 26 : letter;
...
function getCardId(value) {
  ...
  const card = cards.indexOf(value);
  return card;
}
